...
class Haki extends Admin_Controller
{
	...
	public function tambah_dokumen($kat = 0, $id_haki = 0)
	{
	...
	public function tambah_dokumen_proses()
	{
		if ($this->input->post('submit')) {
			...
			$upload_path = './uploads/dokumen/haki';
			...
			$this->db->where('id_haki', $this->input->post('id_haki'));
			$result = $this->db->update('haki', array($kat => $file));

			if ($result) {
				$this->session->set_flashdata('msg', 'Data telah disimpan!');
				redirect(base_url('admin/haki/detail/' . $this->input->post('id_haki')));
			}
		}
	}
